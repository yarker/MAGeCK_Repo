...
class VisualRCount:
  ...
  def writeCountSummaryToTxt(self,txtfile):
    ...
    print('\t'.join(['File','Label','Reads','Mapped', 'Percentage','TotalsgRNAs',  'Zerocounts','GiniIndex']),file=ofstr)
    for i in range(nsp):
      print('\t'.join([self.fastqfile[i],self.fastqlabels[i],str(self.reads[i]),str(self.mappedreads[i]), "{:.4g}".format(self.mappedreads[i]*1.0/self.reads[i]),str(self.totalsgrnas[i]),str(self.zerocounts[i]),"{:.4g}".format(self.gini[i])]),file=ofstr)
    ofstr.close()
